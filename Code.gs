...
function getLeaveData() {
  var sheet = SpreadsheetApp.openById('1qQFFj7iRi4AFJsOKc1xpKbrenitmPD4Evi-cvgmm3UA').getSheetByName('leave');
  var data = sheet.getDataRange().getValues();
...
function updateLeaveStatus(rowIndex, status) {
  var sheet = SpreadsheetApp.openById('1qQFFj7iRi4AFJsOKc1xpKbrenitmPD4Evi-cvgmm3UA').getSheetByName('leave'); // Replace with your sheet name
...
function getLeaveCounts() {
  ...
  var totalLeaves = data.length; // Subtract header row
  var pendingLeaves = data.filter(row => row[7] === "Pending").length; // Assuming the status is in the 8th column
  return {
    total: totalLeaves,
...
function generateReport(reportType, department) {
  ...
  switch (reportType) {
    case 'employee_demographics':
      reportData = generateEmployeeDemographicsReport(department === 'all' ? rows : filteredRows, headers);
      break;
...
function generateDepartmentalHeadcountTurnoverReport(rows, headers) {
  ...
  return {
    turnover: {
      labels: ['Total Employees', 'Turnover Count'],
      values: [totalEmployees, turnoverCount],
...
function addRecord(formData) {
  ...
  var sheet = SpreadsheetApp.openById(sheetId).getSheetByName('Data'); // Ensure correct capitalization of sheet name
  Logger.log(formData);
  sheet.appendRow([
    formData.empId,
    formData.name,
